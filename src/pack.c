...
static void free_entries(git_pack_cache *cache)
{
...
int git_packfile_cache_clear(struct git_pack_file *p)
{
	git_pack_cache *cache = &p->bases;
	if (git_mutex_lock(&cache->lock) < 0) {
		giterr_set(GITERR_OS, "failed to lock cache");
		return -1;
	}
	...
	/* Not allocated yet */
	if (p->bases.entries == NULL)
		return 0;

	free_entries(&p->bases);
